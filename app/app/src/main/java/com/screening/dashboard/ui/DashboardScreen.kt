...
private const val TOTAL_FRAMES = 5  // Images, Todo, Habits, Calendar, Videos
    ...
    Column(
        modifier = Modifier.fillMaxSize().background(DarkBackground)
            ...
            .onKeyEvent { event ->
                ...
                if (activeFrame == FRAME_VIDEOS) return@onKeyEvent false
                when (event.nativeKeyEvent.keyCode) {
                    KeyEvent.KEYCODE_DPAD_LEFT -> { navigateFrame(-1); true }
                    KeyEvent.KEYCODE_DPAD_RIGHT -> { navigateFrame(1); true }
                    KeyEvent.KEYCODE_DPAD_CENTER, KeyEvent.KEYCODE_ENTER -> {
                        if (activeFrame == FRAME_TODO && state.todos.isNotEmpty()) onToggleTodo(state.todos.first().id); true
                    }
        ...
        Box(modifier = Modifier.weight(1f).fillMaxWidth()) {
            ...
            state.pomodoro?.let { pomo ->
                if (pomo.running || pomo.remaining != pomo.duration) {
                    PomodoroOverlay(pomodoro = pomo, modifier = Modifier.align(Alignment.TopEnd).padding(top = 8.dp, end = 48.dp))
                }
            }
            ...
            if (showQR) {
                Box(modifier = Modifier.align(Alignment.Center).clip(RoundedCornerShape(16.dp)).background(DarkCard.copy(alpha = 0.95f)).padding(32.dp), contentAlignment = Alignment.Center) {
                    Column(horizontalAlignment = Alignment.CenterHorizontally) {
                        ...
                        Text(text = state.serverBaseUrl, style = DashboardTypography.bodyMedium.copy(color = AccentCyan))
        ...
        FrameIndicator(count = TOTAL_FRAMES, current = activeFrame, modifier = Modifier.fillMaxWidth().padding(bottom = 24.dp))
    }
}
